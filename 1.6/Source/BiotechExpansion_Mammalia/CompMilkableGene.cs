...
namespace BTE_MMLA
{
    public class CompMilkableGene : CompHasGatherableBodyResource
	{
		...
		public override void PostExposeData()
		{
			...
			if (produce == null)
				produce = BTE_MMLA_ThingDefOf.Milk;
			if (amount == 0)
				amount = 14;
			if (interval == 0)
				interval = 1;
		...
		protected override bool Active
		{
			get
			{
				...
				return (!this.Props.milkFemaleOnly || pawn == null || pawn.gender == Gender.Female)
					&& geneIsPresent
					&& pawn.ageTracker.AgeBiologicalYears >= BTE_MMLA_GeneDefOf.BTEMa_ExcessMilkProduction.minAgeActive;
			}
		}
		...
		public ThingDef produce = BTE_MMLA_ThingDefOf.Milk;
