	...
	public class Gene_AddImplant : Gene
	{
		...
		public List<Hediff> LinkedHediff
		{
			get
			{
				...
				for (int i = 0; i < hediffs.Count; i++)
				{
					...
					if (hediffs[i].def == def.GetModExtension<BTE.GeneImplantInfo>().hediffImplant)
					{
		...
		public override void Tick()
		{
			...
			if (ticksToRegrow <= 0)
			{
				...
				List<BodyPartRecord> hands = this.pawn.def.race.body.GetPartsWithDef(def.GetModExtension<BTE.GeneImplantInfo>().bodyPartImplant);

				foreach (BodyPartRecord hd in hands)
				{
					if (!this.pawn.health.hediffSet.PartIsMissing(hd))
					{
						Hediff h = this.pawn.health.AddHediff(def.GetModExtension<BTE.GeneImplantInfo>().hediffImplant, hd, null, null);
		...
		public override void PostRemove()
		{
			...
			//				this.pawn.health.AddHediff(BTE_MMLA_HediffDefOf.BTE_Ma_SharpClaws, hd, null, null);
			//			}
			//		}
